...
if (indiaData && indiaData.states) {
  indiaData.states.forEach((item: any) => {
    indiaStatesDistricts[item.state] = item.districts;
  });
}

// Default Data
const defaultWeatherData = [
  { label: "dash.temperature", value: "--", icon: Thermometer, color: "text-amber-500", bg: "bg-amber-500/10" },
  { label: "dash.humidity", value: "--", icon: Droplets, color: "text-blue-500", bg: "bg-blue-500/10" },
  { label: "dash.wind", value: "--", icon: Wind, color: "text-emerald-500", bg: "bg-emerald-500/10" },
  { label: "dash.condition", value: "--", icon: CloudSun, color: "text-slate-400", bg: "bg-slate-500/10" },
];
...
export default function Dashboard() {
  ...
  // Sync the local dropdown state with the global location when the picker opens
  useEffect(() => {
    if (showLocPicker && location.source === "manual") {
      setLocState(location.state || "");
      setLocDistrict(location.district || "");
    } else if (showLocPicker && location.source !== "manual") {
       setLocState("");
       setLocDistrict("");
    }
  }, [showLocPicker, location]);
      ...
      if (location.source !== "none") {
        ...
        if (lat && lon) {
          ...
          if (res.success) {
            ...
            setWeatherData([
              { label: "dash.temperature", value: `${Math.round(c.temperature_2m)}°C`, icon: Thermometer, color: "text-amber-500", bg: "bg-amber-500/10" },
              { label: "dash.humidity", value: `${Math.round(c.relative_humidity_2m)}%`, icon: Droplets, color: "text-blue-500", bg: "bg-blue-500/10" },
              { label: "dash.wind", value: `${Math.round(c.wind_speed_10m)} km/h`, icon: Wind, color: "text-emerald-500", bg: "bg-emerald-500/10" },
              { label: "dash.condition", value: cond.label, icon: IconComponent, color: cond.color, bg: "bg-slate-500/10" },
            ]);
          }
        }
      } else {
    ...
    >
            ...
            <div className="relative w-full sm:w-auto" ref={filterRef}>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="max-w-[150px] truncate">{t(`dash.${filterMode.replace(' ', '')}`) || filterMode}</span>
                </div>
                ...
                >
                  ...
                  <div className="space-y-3">
                    ...
                    {location.source !== "none" && (
                      <button
                        onClick={() => {
                          setLocation({ state: "", district: "", city: "", label: "Location not set", source: "none" });
                          setLocState("");
                          setLocDistrict("");
                          setShowLocPicker(false);
                        }}
                        className="w-full flex items-center justify-center gap-2 py-3 bg-slate-100 dark:bg-slate-800/50 text-rose-500 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-800 transition-all"
                      >
                        <X className="h-4 w-4" />
                        Clear Location
                      </button>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {weatherData.map((item, idx) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t(item.label)}</span>
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-7">
        <motion.div variants={itemVariants} className="col-span-full lg:col-span-4 order-1 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <div>
              <h3 className="text-xl font-black text-slate-900 dark:text-white">{t("dash.activeCrops")}</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">{t("dash.biometricsInfo")}</p>
            </div>
            ...
            {filteredCrops.length === 0 ? (
              <div className="py-12 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium max-w-sm">
                  {(filterMode === "Critical Status" || filterMode === "Attention Status") 
                    ? (filterMode === "Critical Status" ? t("dash.noCultivationsCritical") : t("dash.noCultivationsAttention"))
                    : t("dash.trySelectingDesc")}
                </p>
              </div>
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <p className="text-lg font-black text-slate-900 dark:text-white">
                      {t(`dash.${crop.name.toLowerCase()}`)} <span className="text-sm text-slate-500 font-medium">({t(`dash.${crop.sector.toLowerCase().replace(' ', '')}`)})</span>
                    </p>
                    <div className="flex gap-3 mt-1">
                      <p className="text-xs text-slate-500 dark:text-slate-500 font-bold uppercase tracking-tighter">{t("dash.stageLabel")} {t(`dash.${crop.growthStage.split(' ').map((w, i) => i === 0 ? w.toLowerCase() : w).join('')}`)}</p>
                      <p className="text-xs text-emerald-600 dark:text-emerald-500 font-bold uppercase tracking-tighter">{t("dash.marketLabel")} {crop.marketPrice}</p>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-8 mt-4 sm:mt-0 w-full sm:w-auto justify-between sm:justify-end">
                  <div className="text-right">
                    <p className="text-sm font-black text-slate-900 dark:text-white">{crop.health}% {t("dash.health")}</p>
        ...
        <div className="col-span-full lg:col-span-3 space-y-8 order-2">
          ...
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <div>
              <h3 className="text-xl font-black text-slate-900 dark:text-white">{t("dash.alerts")}</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">{t("dash.alertRequirements")}</p>
            </div>
          ...
          <div className="p-8 space-y-5">
            {[
              { action: undefined, actionKey: undefined, urgency: 1, urgencyLabelKey: "dash.criticalPriority", type: "Danger", titleKey: "dash.earlyBlight", descKey: "dash.earlyBlightDesc", icon: AlertTriangle, color: "text-rose-600", bg: "bg-rose-500/10", border: "border-rose-200/50" },
              { action: "Activate Pump", actionKey: "dash.activatePump", urgency: 2, urgencyLabelKey: "dash.moderatePriority", type: "Warning", titleKey: "dash.irrigationNeeded", descKey: "dash.irrigationDesc", icon: Droplets, color: "text-amber-600", bg: "bg-amber-500/10", border: "border-amber-200/50" },
              { action: "Generate Gate Pass", actionKey: "dash.generateGatePass", urgency: 3, urgencyLabelKey: "dash.lowPriority", type: "Success", titleKey: "dash.harvestPrime", descKey: "dash.wheatSectorDesc", icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-500/10", border: "border-emerald-200/50" }
            ].map((alert, idx) => (
              <motion.div
                key={idx}
                whileHover={{ scale: 1.02 }}
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <h4 className={cn("font-black text-sm", alert.color)}>{t(alert.titleKey)}</h4>
                    <span className={cn("text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider", alert.bg, alert.color)}>
                      {t(alert.urgencyLabelKey)}
                    </span>
                  </div>
                  <p className="text-xs text-slate-600 dark:text-slate-400 mt-1 font-medium leading-relaxed">{t(alert.descKey)}</p>
                  {alert.action && (
                    <button 
                      onClick={() => {
                    ...
                    >
                      ...
                      {alert.actionKey ? t(alert.actionKey) : alert.action}
                    </button>
                  )}
                </div>
              </motion.div>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-2xl font-black">{t("dash.productivityScore")}</h4>
            <p className="text-emerald-50 font-medium">{t("dash.productivityDesc")}</p>
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" onClick={() => setShowIrrigationModal(false)}>
            ...
            >
              <div className="p-8">
                ...
                <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2">{t("dash.confirmAction")}</h3>
                <p className="text-slate-500 dark:text-slate-400 font-medium leading-relaxed mb-8">
                  {t("dash.irrigationWarning1")}<strong className="text-slate-700 dark:text-slate-300">{t("dash.cornField")}</strong>{t("dash.irrigationWarning2")}
                </p>
                <div className="flex gap-4">
                  ...
                  >
                    ...
                    {irrigationStatus === "idle" ? t("dash.activatePump") : irrigationStatus === "loading" ? t("dash.syncing") : t("dash.acknowledged")}
                  </button>
                </div>
              </div>
            </motion.div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" onClick={() => setShowWeatherOverrideModal(false)}>
            ...
            >
              <div className="p-8">
                ...
                <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2">{t("dash.weatherAlert")}</h3>
                <p className="text-slate-500 dark:text-slate-400 font-medium leading-relaxed mb-6">
                  {t("dash.weatherWarning1")}<strong className="text-slate-700 dark:text-slate-300">{t("dash.weatherWarning2")}</strong>{t("dash.weatherWarning3")}<strong className="text-slate-700 dark:text-slate-300">{t("dash.weatherWarning4")}</strong>{t("dash.weatherWarning5")}
                </p>
                <div className="flex gap-4">
                  <button 
            ...
            >
              <div className="p-8">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">{t("dash.gatePassTitle")}</h3>
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">{t("dash.lotId")}</p>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-[1.5rem] p-5 mb-6 space-y-4 border border-slate-100 dark:border-slate-800">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-500 uppercase">{t("dash.commodity")}</span>
                    <span className="text-sm font-black text-slate-900 dark:text-white">{gatePassCrop.name} ({gatePassCrop.sector})</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-500 uppercase">{t("dash.estYield")}</span>
                    <span className="text-sm font-black text-slate-900 dark:text-white">{t("dash.quintals42")}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-500 uppercase">{t("dash.marketRate")}</span>
                    <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">{gatePassCrop.marketPrice}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-500 uppercase">{t("dash.estValue")}</span>
